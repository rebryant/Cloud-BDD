...
chunk_ptr build_var(dword_t dest) {
    ...
    report(4, "Created Var operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_canonize(dword_t dest, ref_t vref) {
    ...
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_CANONIZE, worker, id, 3 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
    op_insert_word(op, (word_t) vref,   0+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created Canonize operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_canonize_lookup(dword_t dest, word_t hash, ref_t vref, ref_t hiref, ref_t loref, bool negate) {
    ...
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_CANONIZE_LOOKUP, worker, id, 5 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
    ...
    report(4, "Created Canonize Lookup operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_retrieve_lookup(dword_t dest, ref_t ref) {
    ...
    word_t worker = choose_hashed_worker(hash);
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_RETRIEVE_LOOKUP, worker, id, 1 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,            OP_HEADER_CNT);
    op_insert_word(op, (word_t) ref,     0+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created Retrieve Lookup operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_ite_lookup(dword_t dest, ref_t iref, ref_t tref, ref_t eref, bool negate) {
    ...
    word_t worker = choose_hashed_worker(hash);
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_ITE_LOOKUP, worker, id, 4 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
    ...
    report(4, "Created ITE Lookup operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    chunk_free(ucp);
    return op;
}
...
chunk_ptr build_ite_recurse(dword_t dest, ref_t vref) {
    word_t worker = choose_some_worker();
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_ITE_RECURSE, worker, id, 7 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,         OP_HEADER_CNT);
    op_insert_word(op, (word_t) vref, 0+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created ITE Recurse operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_ite_store(dword_t dest, word_t iref, word_t tref, word_t eref, bool negate) {
    ...
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_ITE_STORE, worker, id, 5 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,            OP_HEADER_CNT);
    op_insert_word(op, (word_t) iref,    0+OPER_SIZE+OP_HEADER_CNT);
    op_insert_word(op, (word_t) tref,    1+OPER_SIZE+OP_HEADER_CNT);
    op_insert_word(op, (word_t) eref,    2+OPER_SIZE+OP_HEADER_CNT);
    op_insert_word(op, (word_t) negate,  4+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created ITE Store operation.  Worker %u.  Operator ID 0x%lx.", worker, id);
    return op;
}
...
chunk_ptr build_uop_down(dword_t dest, unsigned uid, ref_t ref) {
    ...
    word_t worker = choose_hashed_worker(hash);
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_UOP_DOWN, worker, id, 2 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
...
chunk_ptr build_uop_up(dword_t dest, unsigned uid, ref_t ref) {
    word_t worker = choose_own_worker();
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_UOP_UP, worker, id, 4 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
    op_insert_word(op, (word_t) uid,    0+OPER_SIZE+OP_HEADER_CNT);
    op_insert_word(op, (word_t) ref,    1+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created UOP Up operation.  Uid %u.  Worker %u.  Operator ID 0x%lx.", uid, worker, id);
    return op;
}
...
chunk_ptr build_uop_store(dword_t dest, unsigned uid, ref_t ref) {
    word_t worker = choose_own_worker();
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_UOP_STORE, worker, id, 3 + OPER_SIZE + OP_HEADER_CNT);
    op_insert_dword(op, dest,           OP_HEADER_CNT);
    op_insert_word(op, (word_t) uid,    0+OPER_SIZE+OP_HEADER_CNT);
    op_insert_word(op, (word_t) ref,    1+OPER_SIZE+OP_HEADER_CNT);
    report(4, "Created UOP Store operation.  Uid %u.  Worker %u.  Operator ID 0x%lx.", uid, worker, id);
    return op;
}
...
bool do_ite_store_op(chunk_ptr op) {
	...
	while (ele) {
		...
		word_t op_id = msg_get_dest_op_id(dest);
		unsigned agent = msg_get_dest_agent(dest);
		report(4, "\tSending deferred result %sITE(%s, %s, %s) --> %s.  Agent %u, Op Id 0x%lx",
...
static bool complete_cofactor(uop_mgr_ptr umgr, dword_t dest, unsigned uid, ref_t ref, ref_t hiref, ref_t loref, word_t *valp) {
    /* Auxinfo is a set of literals */
    /* Return value is ref of cofactored function */
    bool done = false;
...
static bool complete_equant(uop_mgr_ptr umgr, dword_t dest, unsigned uid, ref_t ref, ref_t hiref, ref_t loref, word_t *valp) {
    /* Auxinfo is a set of variables */
    /* Return value is ref of shifted function */
    bool done = false;
...
static bool complete_shift(uop_mgr_ptr umgr, dword_t dest, unsigned uid, ref_t ref, ref_t hiref, ref_t loref, word_t *valp) {
    bool done = false;
    /* Return value is ref of shifted function */
