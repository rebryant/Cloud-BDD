...
chunk_ptr build_ifork(dword_t dest, word_t width, word_t val, word_t cnt) {
    ...
    report(3, "Created fork operation.  Worker %u.  Operator Id 0x%lx.  Width %u, val %u, cnt %u",
...
chunk_ptr build_incr(dword_t dest, word_t val, word_t cnt) {
    word_t worker = choose_random_worker();
    word_t id = new_operator_id();
    chunk_ptr op = msg_new_operator(OP_INCR, worker, id, 1*OPER_SIZE + 2 + OP_HEADER_CNT);
    op_insert_dword(op, dest, 0*OPER_SIZE + 0 + OP_HEADER_CNT);
    op_insert_word(op, val,  1*OPER_SIZE + 0 + OP_HEADER_CNT);
    op_insert_word(op, cnt,  1*OPER_SIZE + 1 + OP_HEADER_CNT);
    report(3, "Created incr operation.  Worker %u.  Operator Id 0x%lx.  val %u, cnt %u",
	   worker, id, val, cnt);
    return op;
}
...
bool do_ifork_op(chunk_ptr op) {
	...
	for (i = 0; ok && i < 2; i++) {
	    ...
	    word_t id = msg_get_dheader_op_id(dh);
	    report(5, "Fork op spawned fork op.  width %u, val %u, cnt %u, Id 0x%lx", w, val, cnt, id);
	    if (send_op(fork_op))
...
bool do_incr_op(chunk_ptr op) {
	...
	if (ok)
	    report(5, "Sent incr result %lu.  Agent %u.  Operator Id 0x%lx.  Offset %u", val, agent, operator_id, offset);
	else
	    err(false, "Couldn't send result %lu.  Agent %u.  Operator Id 0x%lx.  Offset %u", val, agent, operator_id, offset);
	chunk_free(result);
    } else {
...
bool do_join_op(chunk_ptr op) {
    ...
    if (ok)
	report(5, "Sent join result %lu.  Agent %u.  Operator Id 0x%lx.  Offset %u", val, agent, operator_id, offset);
    else
	err(false, "Couldn't send result %lu.  Agent %u.  Operator Id 0x%lx.  Offset %u", val, agent, operator_id, offset);
    chunk_free(result);
    return ok;
