...
class MScheme(MProblem):
    ...
    # Update values based on symmetry dependencies
    # List consists of pairs of form (dname, sname)
    # Where dname is the name of the dependent variable
    # and sname is the name of the source variable
    def parseDependencies(self, dependencyList):
        for (dname, sname) in dependencyList:
            dv = BrentVariable().fromName(dname)
            sv = BrentVariable().fromName(sname)
            self.assignment[dv] = self.assignment[sv]
        self.kernelTerms = self.findKernels()
        return self
